...
| `camara` | Vista del exterior, hacia el sureste. Se abre tocando la celda de **sol y luna** de la consola. Degrada con gracia mientras no llegue foto, y marca **FOTO ANTIGUA** si pasa de 15 min (tres capturas perdidas) | `pages/kiosk/CamaraPage.tsx` |
...
| **Configuración por estación** | Nombre/etiqueta, minutos para el aviso de **«sin datos»**, **registro** (cambiar la MAC del equipo) y **sensores WN31** con nombres personalizados (ej. «Sala», «Recámara»). En secundarias además: activar/desactivar el watchdog, **alertas para esa estación** (sus umbrales, en Alertas) y **altitud**. La publicación a redes y MQTT son sólo de la estación principal |
...
| **Calibración** | Toggle global y **por estación** con selector. Offsets: temp (°C), humedad (%), presión (hPa); multiplicadores de viento, lluvia, solar y UV (factor). En **secundarias (GW1100)** solo aparece lo aplicable: **sensor integrado** (temp/humedad, que reporta como *interior*) + **presión** (sin viento/lluvia/solar/UV ni canales WN31) |
