...
def _f(settings, name: str, default: float) -> float:
...
def apply_calibration(data: Dict[str, Any], settings) -> Dict[str, Any]:
    ...
    # Offsets globales legados (compat): se suman a todos si están presentes.
    g_temp = _f(settings, "cal_temp_offset", 0.0)
    g_hum = _f(settings, "cal_humidity_offset", 0.0)

    # Temperatura por sensor (+ global legado)
    for field, attr in _TEMP_OFFSETS.items():
        if result.get(field) is not None:
            off = _f(settings, attr, 0.0) + g_temp
            if off:
    ...
    w_mult = _f(settings, "cal_wind_mult", 1.0)
    if w_mult != 1.0:
